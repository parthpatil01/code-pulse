...
# Run Code Resource and Methods
resource "aws_api_gateway_resource" "refactor_code" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  parent_id   = aws_api_gateway_rest_api.code_editor_api.root_resource_id
  path_part   = "refactor"
}

resource "aws_api_gateway_method" "refactor_code_post" {
  rest_api_id   = aws_api_gateway_rest_api.code_editor_api.id
  resource_id   = aws_api_gateway_resource.refactor_code.id
  http_method   = "POST"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "refactor_lambda_integration" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  resource_id = aws_api_gateway_resource.refactor_code.id
  http_method = aws_api_gateway_method.refactor_code_post.http_method

  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.lambda_refactor_invoke_arn
}

resource "aws_api_gateway_method_response" "refactor_code_post_200" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  resource_id = aws_api_gateway_resource.refactor_code.id
  http_method = aws_api_gateway_method.refactor_code_post.http_method
  status_code = "200"
  ...
  depends_on = [aws_api_gateway_method.refactor_code_post]
}

# OPTIONS Method for CORS
resource "aws_api_gateway_method" "refactor_code_options" {
  rest_api_id   = aws_api_gateway_rest_api.code_editor_api.id
  resource_id   = aws_api_gateway_resource.refactor_code.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "refactor_code_options_integration" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  resource_id = aws_api_gateway_resource.refactor_code.id
  http_method = aws_api_gateway_method.refactor_code_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = jsonencode({
      statusCode = 200
...
resource "aws_api_gateway_method_response" "refactor_code_options_200" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  resource_id = aws_api_gateway_resource.refactor_code.id
  http_method = aws_api_gateway_method.refactor_code_options.http_method
  status_code = "200"
  ...
  depends_on = [aws_api_gateway_method.refactor_code_options]
}

# Integration Response for OPTIONS Method
resource "aws_api_gateway_integration_response" "refactor_code_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.code_editor_api.id
  resource_id = aws_api_gateway_resource.refactor_code.id
  http_method = aws_api_gateway_method.refactor_code_options.http_method
  status_code = aws_api_gateway_method_response.refactor_code_options_200.status_code
  ...
  depends_on = [
    aws_api_gateway_method_response.refactor_code_options_200,
    aws_api_gateway_integration.refactor_code_options_integration
  ]
}
...
resource "aws_api_gateway_deployment" "api_deployment" {
  depends_on = [
    ...
    aws_api_gateway_integration.refactor_lambda_integration,
    aws_api_gateway_integration.refactor_code_options_integration,
    aws_api_gateway_integration_response.refactor_code_options_integration_response
  ]
